...
## Conversations flagged for human oversight

When your oversight rules decide a conversation warrants human attention, it's flagged for review and a bar appears at the top of the conversation. The bar is where the reviewer records what they concluded — **Leave as it is** or **I stepped in** — with an optional note.

Recording a decision resolves the flag, so the conversation leaves the review queue. The bar stays, collapsed to a single line naming the conclusion, who reached it and when, so the review is still visible afterwards rather than vanishing along with the flag.

The rules that raise the flag are configured in [Conversations settings](./settings/conversations.mdx); the decision belongs on the conversation itself. For the full workflow, see [Record oversight decisions](/operational-guides/human-oversight).
