...
The governance module gives administrators observability and accountability over AI model usage.  It provides dashboards and detailed logs to track costs, token consumption, inference performance and conversational activity.

## Overview

Open **Governance** from the top navigation bar of the Control Room.  Two views are available:

- **Dashboard:** aggregated usage metrics and performance for the selected data type;
- **Audit Log:** a detailed, filterable table of individual inference or conversation events.

Both views support the same set of filters: type, touchpoints, models, and period.&#x20;
...
- **Total costs**: cumulative cost of API calls or conversations over the selected period, shown in euros. Displays `0,00 €` if cost tracking is disabled.
- **Total inferences**: number of API calls or conversation interactions.
- **Total input tokens**: total tokens sent to models.
- **Total output tokens**: total tokens returned by models.

All card updates automatically when filters change.
