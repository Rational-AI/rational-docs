...
Workflows automate data transformation in Rational AI. This guide covers creating, configuring, and testing workflows that convert raw data into AI-ready knowledge using AI.

import Admonition from "@theme/Admonition";

<Admonition type="info" icon="📖" title="WHAT IS A WORKFLOW?">  
Workflows automate the transformation of raw data into AI-ready knowledge. They extract key information, augment it, identify connections between data points, and structure everything into a queryable knowledge base that your AI applications can use.
</Admonition>

## Prerequisites

Before creating workflows, ensure you have:
- Permissions to access to the AI Control Room Settings area
- A Knowledge base created (workflows operate on Knowledge bases)
- Python and `uv` package manager installed ([installation guide](https://docs.astral.sh/uv/))

## Access the workflow interface

Navigate to **Settings** → **Workflows** from the left sidebar. This is the central workspace for creating and managing workflows and processing rules.

<img
  src={require('@site/static/img/screens/workflows.png').default}
  alt="Workflow management interface showing the workflow list and editor"
  style={{ 
    width: '100%', 
    maxHeight: '100%', 
    objectFit: 'contain',
...
Between the **Description** field and the **Code** editor, the **New workflow** and **Edit workflow** modals include an **Extraction capabilities** section. This is where you tag a workflow with the kinds of content it pulls out of a document — such as **Images**, **Tables**, or **Annotations**. The tags don't change what the workflow does; they describe it, so the platform can surface the right workflow in the right place.

Select capabilities by toggling their chips:

- **Choose any number.** Click a chip to select it, click it again to clear it. A workflow can carry no capabilities, exactly one, or several.
- **Saved with the workflow.** Your selection is included in the payload when you click **Add**, or **Save** when editing.

<Admonition type="note" icon="📝" title="THE CAPABILITY LIST IS DYNAMIC">
The available capabilities aren't hardcoded in the interface — Rational AI fetches the master list (Images, Tables, Annotations, and any future additions) from the backend, so the set can grow without a front-end change.
</Admonition>

**On the workflow detail view**, the capabilities you assigned appear again as read-only, non-clickable chips directly below the description — a quick summary of what the workflow is tagged to extract. To change them, edit the workflow and toggle the chips.

**Capability tags also filter where you pick a workflow.** When the context requires a capability — for example choosing a workflow from the **Image** tool settings, or running **Auto-annotate images** — the list is filtered to show only workflows that carry the matching tag, so you can't select one that can't do the job. Where no specific capability is required, every workflow is listed.
...
def process(document: File, options: dict):
    ...
    # Get the Knowledge instance this document belongs to
    knowledge = Knowledge(document.knowledge_id)

    # Create a new Rational Resource in the Knowledge Base
...
This is not only possible, but also encouraged, as it allows to use your favorite IDE and debugging tools.
...
```
Then, we need to prepare a Json file as input, let's call it `workflow_input.json`. This file needs to have this structure:
```json
{
