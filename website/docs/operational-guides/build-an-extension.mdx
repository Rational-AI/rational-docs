...
manifest fills in. The main types are:
...
The manifest also supports `ruleProviders`, `knowledgeProviders` and
`widgetProviders`. Include the required `channelProviders` and
`mcpServerProviders` arrays even when empty; unused optional provider arrays
can be omitted.

## Anatomy of an extension

Keep one stable source directory per extension, such as `my-extension/` (or
`extension/` in a single-extension repository), with a standard layout:
...
| `nameIdentifier` | yes | A short machine slug, e.g. `time`, `github-remote`. Determines the archive filename and inner root prefix. |
| `version` | yes | Semantic release version (`1.0.0`); determines the archive filename and inner root suffix. |
...
Build the extension on your own computer or in CI. Many example projects
provide a `make-tar.sh` script in their directory. This is a project convention,
...
Build any server dependencies and code first. The packaging step must read
`nameIdentifier` and `version` from `manifest.json`, producing
`my-extension-1.0.0.tar.bz2` with a single inner root named
`my-extension-1.0.0/`. Runtime paths such as `dist` remain relative to that
root. The source directory stays `my-extension/` across releases: update the
manifest version and keep source history in Git, without copying old version
folders alongside the current source.

For an extension whose runtime files are already built in `dist/`, this
`make-tar.sh` example uses Python 3 to package the required files. Add other
assets referenced by your manifest to `payload` as needed. Run it with
`sh make-tar.sh`; it works regardless of the caller's working directory.
...
identity = [manifest["nameIdentifier"], manifest["version"]]
if not all(isinstance(value, str) and
           re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9._-]*", value)
           for value in identity):
    raise ValueError("Invalid package identity")
package_name = "-".join(identity)
payload = [Path("manifest.json"), Path("icon.png"), Path("dist")]
for path in payload:
    if not path.exists():
        raise FileNotFoundError(path)
# Keep archives outside dist/, which contains the runtime payload.
output = Path("artifacts")
output.mkdir(exist_ok=True)
with tarfile.open(output / f"{package_name}.tar.bz2", "w:bz2") as archive:
    for path in payload:
        archive.add(path, arcname=f"{package_name}/{path}")
print(output / f"{package_name}.tar.bz2")
PYTHON
```

Exclude generated `artifacts/` from Git. A **remote** extension skips packaging
entirely — its `manifest.json` *is* the distributable.
...
UUID). Derive the archive filename and inner root from `nameIdentifier` and
`version`; keep the source directory name stable.
</Admonition>

## Develop an installed extension

You do not need access to the Rational AI server filesystem or Kubernetes.
Build locally and upload through the UI or tenant management API. For individual
file edits, remote previews and bringing changes back into Git, see
